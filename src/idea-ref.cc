...
ushort multiply(long a, long b)
{
  ...
  if (a == 0) a = 65536l;		  // 0x10000 = 2^n
  if (b == 0) b = 65536l;
  c = a * b;
  if(c == 0) {
	 printf("[%s:%d] %8lX %8lX %8lX\n", __FILE__, __LINE__, a, b, c);
	 assert(a == b);
	 assert(a == 0x10000);
  }
  assert(c != 0);
  if (c) {							  // c == 0if a = b = 0x10000 = 2^n
    ...
    if (cl >= ch) {
		if((cl - ch) > 0xFFFF) {
		  printf("[%s:%d] %8lX %8lX %8lX\n", __FILE__, __LINE__, cl, ch, (cl - ch));
		}
		assert((cl - ch) <= 0xFFFF);
		return (ushort) (cl - ch);
	 }
    return (ushort) ((cl - ch + 65537l) & 65535l); // 65537l == 0x10001 = 2^n + 1
  }
  if (a == b) return 1;
  return 0;
}

void IDEA_encryption(ushort *X_in, ushort *Y, long **K)
{
  assert(NROUNDS <= 8);
  uint32_t nrounds = NROUNDS;
  ushort a, r, t0, t1, t2;

  ushort X[4] = {X_in[0], X_in[1], X_in[2], X_in[3]};
  ...
  Y[2] = add(X[1], K[nrounds][2]);
#endif
#ifdef DEBUG
  printf("%d ", nrounds + 1);
  for (uint32_t i = 0; i < 6; i++) {
	 //    printf("%4x ", (ushort) K[8][i]);
    printf("%6d ", (ushort) K[nrounds][i]);
  }
  printf(" | %4x %4x %4x %4x\n\n", Y[0], Y[1], Y[2], Y[3]);
 #endif
}

// Linearized version of IDEA w.r.t. ADD differences i.e.
// all XOR-s are replaced by the ADD operation
void IDEA_LIN_encryption(ushort *X_in, ushort *Y, long **K)
{
  assert(NROUNDS <= 8);
  uint32_t nrounds = NROUNDS;
  ushort a, r, t0, t1, t2;
  ...
  for (r = 0; r < nrounds; r++) {
    ...
    X[1] = X[2] ^ t1;
    X[2] = a;
#else	 // ADD-LINEAR IDEA
	 // {start of MA structure
    t0 = MUL(K[r][4], add(X[0],X[2]));
    t1 = MUL(K[r][5], add(t0, add(X[1], X[3])));
    t2 = add(t0, t1);
	 // end of MA structure}
    X[0] = add(X[0], t1);
    X[3] = add(X[3], t2);
  ...
  Y[2] = add(X[1], K[nrounds][2]);
#else // ADD-LINEAR IDEA
  Y[0] = MUL(X[0], K[nrounds][0]);
  Y[3] = MUL(X[3], K[nrounds][3]);
  Y[1] = add(X[2], K[nrounds][1]);
  Y[2] = add(X[1], K[nrounds][2]);
#endif // original IDEA
#endif
