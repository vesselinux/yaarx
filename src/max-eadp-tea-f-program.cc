 ...
 * \date 2012-2013
 * \brief The probability \f$\max_{dd} \mathrm{eadp}^{F}(da \rightarrow dd)\f$ with user-provided input.
 */ 
#ifndef COMMON_H
#include "common.hh"
#endif
...
/**
 * Program for computing \f$\max_{dd}~\mathrm{eadp}^{F}\f$ with user-provided
 * input.
 */
void max_eadp_tea_f_program()
{
  ...
  printf("MAX_EADP_TEA_F: n = %d. Enter dx (hex). Ctrl-D to exit:\n", WORD_SIZE);
  while (EOF != scanf("%x", &dx)) {
	 //			 printf("%x %x %x %x\n", da, db, dc, dd);
	 dx &= MASK;
	 double p1 = max_eadp_tea_f(A, dx, &dy, &p1, lsh_const, rsh_const);
	 assert((p1 >= 0.0) && (p1 <= 1.0));
	 printf("[%s:%d] %2d %2d | MAX_EADP_TEA_F(%8X -> %8X) = %6.5f\n", __FILE__, __LINE__, lsh_const, rsh_const, dx, dy, p1);
  }
  printf("Exiting...\n");
...
/** 
 * Main function for the \f$\max_{dd}~\mathrm{eadp}^{F}\f$ program.
 */ 
int main()
{
	 srandom(time(NULL));
